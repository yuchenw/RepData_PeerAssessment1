...
download.file(url = URL, destfile = "Activity_monitoring_data.zip", method="curl")
...
The mean is **`r paste(format(Sum_steps_mean))`**. The median is **`r paste(format(Sum_steps_median))`**. 
...
The interval with the maximum value of mean is **`r paste(format(Interval_Max ))`**.
...
The total number of missing values in `steps` is **`r paste(format(NA_Number[1]))`**.
